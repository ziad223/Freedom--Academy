...
const Germany = () => {
  ...
  const germanyUsers = useSelector(state =>state.germany.users);
  ...
  return (
    <div className='room-three'>
        ...
        onClick={() =>{dispatch(addUser({id :germanyUsers.length + 1 ,name : name , phone : phone , course : course}))
      setName("");
      setCourse("");
      setPhone("")
      }}
    ...
    {germanyUsers.length > 0 ?  <table border = '2' cellPadding = '10'>
        <tr>
            <th>id</th>
            <th>name</th>
            <th>Phone Number</th>
       ...
       {germanyUsers.length > 0 &&  germanyUsers.map((user) =>{
       return (
        <tr key={user.id}>
