...
const Graphic = () => {
  ...
  const graphicUsers = useSelector(state =>state.germany.users);
  ...
  return (
    <div className='cancel'>
        ...
        onClick={() =>{dispatch(addUser({id :graphicUsers.length + 1 ,name : name , phone : phone , course : course}))
      setName("");
      setCourse("");
      setPhone("")
      }}
    ...
    {graphicUsers.length > 0 ?  <table border = '2' cellPadding = '10'>
       ...
       {graphicUsers.length > 0 &&  graphicUsers.map((user) =>{
       return (
        <tr key={user.id}>
